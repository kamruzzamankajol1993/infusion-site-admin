<script>
    ...
    var canUpdate = @json(Auth::user()->can('digitalMarketingGrowthUpdate'));
    var canDelete = @json(Auth::user()->can('digitalMarketingGrowthDelete'));
    var routes = {
        fetch: "{{ route('ajax.digitalMarketingGrowth.data') }}",
        show: id => `{{ route('digital-marketing-growth.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('digital-marketing-growth.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('digital-marketing-growth.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('digitalMarketingGrowth.updateOrder') }}",
        token: "{{ csrf_token() }}"
    };
        ...
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            res.data.forEach((item, i) => {
                ...
                rows += `<tr>
                    ...
                    <td>${item.title || 'N/A'}</td> <td>${item.order}</td> <td>${editBtn}${delBtn}</td>
                </tr>`;
            });
            $('#tableBody').html(rows);
    ...
    $(document.body).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), data => {
            $('#editId').val(data.id); $('#editTitle').val(data.title); editModal.show();
