<script>
    ...
    var canUpdate = @json(Auth::user()->can('webSolutionChecklistUpdate')); // Adjust permission
    var canDelete = @json(Auth::user()->can('webSolutionChecklistDelete')); // Adjust permission
    var routes = {
        fetch: "{{ route('webSolution.checklist.data') }}",
        show: id => `{{ route('webSolution.checklist.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('webSolution.checklist.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('webSolution.checklist.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('webSolution.checklist.updateOrder') }}",
        token: "{{ csrf_token() }}"
    };
