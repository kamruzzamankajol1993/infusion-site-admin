<script>
    ...
    var canUpdate = @json(Auth::user()->can('graphicDesignSolutionUpdate'));
    var canDelete = @json(Auth::user()->can('graphicDesignSolutionDelete'));
    var routes = {
        fetch: "{{ route('graphicDesign.solution.data') }}",
        show: id => `{{ route('graphicDesign.solution.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('graphicDesign.solution.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('graphicDesign.solution.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('graphicDesign.solution.updateOrder') }}",
        token: "{{ csrf_token() }}",
        assetBase: "{{ asset('') }}"
    };
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            res.data.forEach((item, i) => {
                let iconHtml = item.icon ? `<img src="${routes.assetBase}${item.icon}" alt="${item.title}" style="width: 50px; height: 50px; object-fit: contain;">` : ``;
