...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        ...
        <div>
            @if (Auth::user()->can('facebookAdsFaqAdd'))
            <button type="button" data-bs-toggle="modal" data-bs-target="#addModal" class="btn text-white" style="background-color: var(--primary-color); white-space: nowrap;">
                <i data-feather="plus" class="me-1" style="width:18px; height:18px;"></i> Add New FAQ
            </button>
            @endif
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
                <li class="nav-item" role="presentation"><a class="nav-link {{ $activeTab === 'table' ? 'active' : '' }}" href="{{ route('facebookAds.faq.index', ['tab' => 'table']) }}" id="table-tab"><i data-feather="list" class="me-1" style="width:16px;"></i> Data Table</a></li>
                @if (Auth::user()->can('facebookAdsFaqUpdate'))
                <li class="nav-item" role="presentation"><a class="nav-link {{ $activeTab === 'reorder' ? 'active' : '' }}" href="{{ route('facebookAds.faq.index', ['tab' => 'reorder']) }}" id="reorder-tab"><i data-feather="move" class="me-1" style="width:16px;"></i> Drag & Drop Reorder</a></li>
                @endif
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="myTabContent">
                <div class="tab-pane fade {{ $activeTab === 'table' ? 'show active' : '' }}" id="table-tab-pane" role="tabpanel" aria-labelledby="table-tab">
                    <div class="d-flex justify-content-end mb-3"><div class="input-group" style="width: 250px;"><input type="text" class="form-control" id="searchInput" placeholder="Search by question..."><span class="input-group-text"><i data-feather="search" style="width:16px;"></i></span></div></div>
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered align-middle">
                            <thead class="table-light">
                                <tr>
                                    <th style="width: 5%;">Sl</th>
                                    <th class="sortable" data-column="question" style="width: 40%;">Question</th>
                                    <th style="width: 30%;">Answer (Plain Text)</th>
                ...
                <div class="tab-pane fade {{ $activeTab === 'reorder' ? 'show active' : '' }}" id="reorder-tab-pane" role="tabpanel" aria-labelledby="reorder-tab">
                    @if($activeTab === 'reorder' && !$items->isEmpty())
                        ...
                        <ul id="sortableList">
                            ...
                            <li data-id="{{ $item->id }}">
                                <div><span class="sortable-handle"><i class="bi bi-grip-vertical"></i></span> <strong>{{ $item->question }}</strong></div>
                    ...
                    @else
                        <div class="alert alert-info text-center">No FAQs found to reorder.</div>
                    @endif
                </div>
            </div>
        </div>
...
@include('admin.facebook_ads_faq._partial.addModal')
@include('admin.facebook_ads_faq._partial.editModal')
@endsection

@section('script')
<script src="https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/summernote@0.8.18/dist/summernote-lite.min.js"></script>
@include('admin.facebook_ads_faq._partial.script')
@endsection