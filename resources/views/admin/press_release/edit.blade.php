...
<div class="container-fluid px-4 py-4">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-4">
            ...
            <li class="breadcrumb-item active" aria-current="page">Edit: {{ Str::limit($pressRelease->title, 40) }}</li>
        </ol>
    </nav>

    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <h5 class="card-title mb-0">Edit Press Release</h5>
        </div>
        <div class="card-body">
            @include('flash_message')

            <form action="{{ route('pressRelease.update', $pressRelease->id) }}" method="POST" enctype="multipart/form-data" novalidate id="editPressReleaseForm">
                @csrf
                @method('PUT')
                <div class="row g-3">
                     {{-- Title --}}
                    <div class="col-12">
                        ...
                        <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $pressRelease->title) }}" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <small class="form-text text-muted">Size: 1200x800 px, Max: 2MB. Leave blank to keep current.</small>
                        @error('image') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
                        {{-- Image Preview --}}
                        <div class="image-preview-box">
                            @if($pressRelease->image)
                                {{-- Use asset() helper directly on the image path --}}
                                <img id="imagePreviewPR" src="{{asset($pressRelease->image)}}" alt="Current Image">
                                <span class="placeholder-text" style="display:none;">Image Preview<br>(1200 x 800)</span>
                             @else
                    ...
                    <div class="col-md-6">
                         ...
                         <div class="mb-3">
                            ...
                            {{-- Check if release_date exists before formatting --}}
                            <input type="text" class="form-control datepicker @error('release_date') is-invalid @enderror" id="release_date" name="release_date" value="{{ old('release_date', $pressRelease->release_date ? $pressRelease->release_date->format('Y-m-d') : '') }}" placeholder="YYYY-MM-DD" autocomplete="off">
                         ...
                         <div class="mb-3" id="linkField">
                            ...
                            <input type="url" class="form-control @error('link') is-invalid @enderror" id="link" name="link" value="{{ old('link', $pressRelease->link) }}" placeholder="https://example.com/press-release">
                    ...
                    <div class="col-12" id="descriptionField">
                        ...
                        <textarea class="form-control summernote @error('description') is-invalid @enderror" id="description" name="description">{{ old('description', $pressRelease->description) }}</textarea>
...
<script>
    $(document).ready(function() {
        ...
        $("#image").change(function() {
            const input = this; 
            const preview = $('#imagePreviewPR'); 
            const placeholder = $('.placeholder-text');
            // Use the asset path for the original image
            const originalSrc = '{{ $pressRelease->image ? asset($pressRelease->image) : "" }}';
        ...
        $('form#editPressReleaseForm').submit(function(e) {
            let isValid = true; 
            let $form = $(this);
            $('.form-error').empty(); 
         ...
         $('form#editPressReleaseForm input, form#editPressReleaseForm select, form#editPressReleaseForm textarea').on('input change', function() {
