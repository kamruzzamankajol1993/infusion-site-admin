<script>
    ...
    var canUpdate = @json(Auth::user()->can('ukTestimonialUpdate'));
    var canDelete = @json(Auth::user()->can('ukTestimonialDelete'));
    var routes = {
        fetch: "{{ route('ukCompany.testimonial.data') }}",
        show: id => `{{ route('ukCompany.testimonial.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('ukCompany.testimonial.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('ukCompany.testimonial.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('ukCompany.testimonial.updateOrder') }}",
        token: "{{ csrf_token() }}",
        assetBase: "{{ asset('') }}"
    };
    ...
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            if (!items || items.length === 0) {
                 rows = `<tr><td colspan="7" class="text-center text-muted">No testimonials found.</td></tr>`;
            ...
            items.forEach((item, i) => {
                ...
                let img = item.image ? `<img src="${routes.assetBase}${item.image}" alt="${item.name}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 50%;">` : 'N/A';
                let rating = renderStars(item.rating);
                ...
                rows += `<tr>
                    <td>${sl}</td> <td>${img}</td> <td>${item.name}</td> <td>${item.designation || 'N/A'}</td>
                    <td>${rating}</td> <td>${item.order}</td> <td>${editBtn}${delBtn}</td>
                </tr>`;
            });
            $('#tableBody').html(rows);
    ...
    $(document.body).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), data => {
            ...
            editModal.show();
        }).fail(() => Swal.fire('Error!', 'Could not fetch testimonial data.', 'error'));
    });
    
    $(document).on('click', '.btn-delete', function () {
        const id = $(this).data('id');
        Swal.fire({ title: 'Delete this testimonial?', text: "You won't be able to revert this!", icon: 'warning',
