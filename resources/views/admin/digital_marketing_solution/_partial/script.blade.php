<script>
    ...
    var canUpdate = @json(Auth::user()->can('digitalMarketingSolutionUpdate'));
    var canDelete = @json(Auth::user()->can('digitalMarketingSolutionDelete'));
    var routes = {
        fetch: "{{ route('ajax.digitalMarketingSolution.data') }}",
        show: id => `{{ route('digital-marketing-solution.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('digital-marketing-solution.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('digital-marketing-solution.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('digitalMarketingSolution.updateOrder') }}",
        token: "{{ csrf_token() }}"
    };
    ...
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            if (!res.data || res.data.length === 0) {
                 rows = `<tr><td colspan="6" class="text-center text-muted">No solutions found.</td></tr>`;
                 $('#tableBody').html(rows); $('#tableRowCount').text(`Showing 0 to 0 of 0 entries`); $('#pagination').empty(); return;
            }
            res.data.forEach((item, i) => {
                let iconHtml = item.icon ? `<img src="{{ asset('') }}${item.icon}" alt="${item.title}" style="width: 50px; height: 50px; object-fit: contain;">` : ``;
    ...
    function renderPagination(res) { 
        ...
        if (res.last_page > 1) {
            ...
            const start = Math.max(1, res.current_page - 2); const end = Math.min(res.last_page, res.current_page + 2);
    ...
    $(document).on('click', '.sortable', e => {
        ...
        $('.sortable').removeClass('sorting_asc sorting_desc'); $(e.currentTarget).addClass(sortDirection === 'asc' ? 'sorting_asc' : 'sorting_desc'); 
        fetchData();
    });
    $(document).on('click', '.page-link', e => {
        e.preventDefault(); const page = parseInt($(e.currentTarget).data('page'));
        if (!isNaN(page) && page !== currentPage) { currentPage = page; fetchData(); }
    });

    $(document.body).on('click', '.btn-edit', function () {
        const id = $(this).data('id');
        const preview = $('#editIconPreview'); const placeholder = $('#editPreviewPlaceholder');
        $('#editForm').attr('action', routes.update(id));
        $.get(routes.show(id), data => {
            $('#editId').val(data.id); $('#editTitle').val(data.title); $('#editDescription').val(data.description);
            if (data.icon_url) {
                preview.attr('src', data.icon_url).show(); preview.attr('data-original-src', data.icon_url); placeholder.hide();
    ...
    $(document).on('click', '.btn-delete', function () {
        ...
        Swal.fire({ title: 'Delete this item?', text: "You won't be able to revert this!", icon: 'warning',
            ...
            if (result.isConfirmed) { $('#delete-form').attr('action', routes.delete(id)).submit(); }
        });
    });
