<script>
    ...
    var canUpdate = @json(Auth::user()->can('facebookMoreServiceUpdate'));
    var canDelete = @json(Auth::user()->can('facebookMoreServiceDelete'));
    var routes = {
        fetch: "{{ route('facebookPage.service.data') }}",
        show: id => `{{ route('facebookPage.service.show', ':id') }}`.replace(':id', id),
    ...
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            if (!items || items.length === 0) {
                 rows = `<tr><td colspan="6" class="text-center text-muted">No services found.</td></tr>`;
