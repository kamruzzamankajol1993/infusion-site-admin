<script>
    ...
    var canUpdate = @json(Auth::user()->can('webSolutionProvidingUpdate'));
    var canDelete = @json(Auth::user()->can('webSolutionProvidingDelete'));
    var routes = {
        fetch: "{{ route('webSolution.providing.data') }}",
        show: id => `{{ route('webSolution.providing.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('webSolution.providing.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('webSolution.providing.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('webSolution.providing.updateOrder') }}",
        token: "{{ csrf_token() }}",
        assetBase: "{{ asset('') }}"
    };
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            res.data.forEach((item, i) => {
                let img = `<img src="${routes.assetBase}${item.image}" alt="${item.title}" style="width: 100px; height: 67px; object-fit: cover;">`;
                ...
                rows += `<tr>
                    ...
                    <td>${img}</td> <td>${item.title || 'N/A'}</td> <td>${item.order}</td> <td>${editBtn}${delBtn}</td>
                </tr>`;
            });
            $('#tableBody').html(rows);
            const start = (res.current_page - 1) * res.per_page + 1, end = start + res.data.length - 1;
            $('#tableRowCount').text(`Showing ${start} to ${end} of ${res.total} entries`);
            renderPagination(res);
        });
    }
    function renderPagination(res) { /* ... same as previous script ... */ 
        let links = '';
        if (res.last_page > 1) {
    ...
    $(document).on('click', '.sortable', e => {
        let col = $(e.currentTarget).data('column'); sortDirection = sortColumn === col ? (sortDirection === 'asc' ? 'desc' : 'asc') : 'asc'; sortColumn = col;
        $('.sortable').removeClass('sorting_asc sorting_desc'); $(e.currentTarget).addClass(sortDirection === 'asc' ? 'sorting_asc' : 'sorting_desc'); fetchData();
    });
    ...
    $(document.body).on('click', '.btn-edit', function () {
        const id = $(this).data('id'); const preview = $('#editImagePreview'); const placeholder = $('#editPreviewPlaceholder');
        $('#editForm').attr('action', routes.update(id));
        $.get(routes.show(id), data => {
            $('#editId').val(data.id); $('#editTitle').val(data.title);
            $('#editButtonText').val(data.button_text); $('#editButtonLink').val(data.button_link);
            if (data.image_url) {
                preview.attr('src', data.image_url).show(); preview.attr('data-original-src', data.image_url); placeholder.hide();
            } else { preview.hide(); placeholder.show(); }
            editModal.show();
        }).fail(() => Swal.fire('Error!', 'Could not fetch item data.', 'error'));
    });
    $(document).on('click', '.btn-delete', function () { /* ... same as previous script ... */ 
        const id = $(this).data('id');
        Swal.fire({ title: 'Delete this item?', text: "You won't be able to revert this!", icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', confirmButtonText: 'Yes, delete it!'
        }).then(result => { if (result.isConfirmed) { $('#delete-form').attr('action', routes.delete(id)).submit(); } });
    });
    const sortableList = document.getElementById('sortableList');
    if (sortableList) { /* ... same as previous script ... */ 
