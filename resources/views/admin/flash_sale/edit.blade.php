    ...
    <div class="container-fluid">
        <div class="py-4">
             <div class="d-flex justify-content-between align-items-center">
                <h2 class="mb-0">Edit: {{ $flashSale->title }}</h2>
        ...
        <form action="{{ route('flash-sales.update', $flashSale->id) }}" method="POST">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            ...
                            <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $flashSale->title) }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="start_date" class="form-label">Start Date & Time <span class="text-danger">*</span></label>
                            <input type="text" name="start_date" id="start_date" class="form-control" value="{{ old('start_date', $flashSale->start_date->format('Y-m-d H:i')) }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="end_date" class="form-label">End Date & Time <span class="text-danger">*</span></label>
                            <input type="text" name="end_date" id="end_date" class="form-control" value="{{ old('end_date', $flashSale->end_date->format('Y-m-d H:i')) }}" required>
                        </div>
                        <div class="col-md-12">
                            <div class="form-check form-switch">
                                <input class="form-check-input" type="checkbox" role="switch" id="status" name="status" value="1" @checked($flashSale->status)>
                            ...
                            <tbody id="selected-products-table">
                                @forelse($flashSale->products as $product)
                                    <tr id="product-row-{{ $product->id }}">
                                        ...
                                        <td><input type="number" name="products[{{ $product->id }}][flash_price]" class="form-control form-control-sm" required step="0.01" min="0" value="{{ $product->pivot->flash_price }}"></td>
                                        <td><input type="number" name="products[{{ $product->id }}][quantity]" class="form-control form-control-sm" required min="1" value="{{ $product->pivot->quantity }}"></td>
                                ...
                                @empty
                                    <tr id="no-products-row"><td colspan="5" class="text-center text-muted">No products added yet.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
            ...
            <div class="text-end mt-4">
                <button type="submit" class="btn btn-primary btn-lg">
                    <i data-feather="save" class="me-1" style="width: 18px;"></i> Update Flash Sale
                </button>
            </div>
        </form>
    </div>
