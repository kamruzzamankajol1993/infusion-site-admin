<script>
    ...
    var canUpdate = @json(Auth::user()->can('ukReviewPlatformUpdate'));
    var canDelete = @json(Auth::user()->can('ukReviewPlatformDelete'));
    var routes = {
        fetch: "{{ route('ukCompany.reviewPlatform.data') }}",
        show: id => `{{ route('ukCompany.review-platform.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('ukCompany.review-platform.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('ukCompany.review-platform.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('ukCompany.reviewPlatform.updateOrder') }}",
        token: "{{ csrf_token() }}",
        assetBase: "{{ asset('') }}"
    };
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            if (!items || items.length === 0) {
                 rows = `<tr><td colspan="6" class="text-center text-muted">No platforms found.</td></tr>`;
            ...
            items.forEach((item, i) => {
                ...
                let img = `<img src="${routes.assetBase}${item.image}" alt="${item.name}" style="width: 150px; height: 45px; object-fit: contain;">`;
                ...
                rows += `<tr>
                    <td>${sl}</td> <td>${img}</td> <td>${item.name}</td> <td>${item.rating_text}</td>
    ...
    $(document.body).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), data => {
            ...
            $('#editRatingText').val(data.rating_text);
            $('#editReviewLink').val(data.review_link);
            if (data.image_url) {
            ...
            editModal.show();
        }).fail(() => Swal.fire('Error!', 'Could not fetch platform data.', 'error'));
    });
    
    $(document).on('click', '.btn-delete', function () {
        const id = $(this).data('id');
        Swal.fire({ title: 'Delete this platform?', text: "You won't be able to revert this!", icon: 'warning',
