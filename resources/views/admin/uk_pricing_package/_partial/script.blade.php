<script>
    ...
    var canUpdate = @json(Auth::user()->can('ukPricingPackageUpdate'));
    var canDelete = @json(Auth::user()->can('ukPricingPackageDelete'));
    var routes = {
        fetch: "{{ route('ukCompany.package.data') }}",
        show: id => `{{ route('ukCompany.package.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('ukCompany.package.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('ukCompany.package.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('ukCompany.package.updateOrder') }}",
        token: "{{ csrf_token() }}"
    };
    function fetchData() {
            ...
            items.forEach((item, i) => {
                ...
                if (Array.isArray(item.features)) {
                    item.features.forEach(f => {
                        let liClass = f.included ? 'included' : 'not-included';
                        featuresHtml += `<li class="${liClass}">${f.text}</li>`;
                    });
                }
                featuresHtml += '</ul>';
        ...
        $.get(routes.show(id), data => {
            ...
            if (Array.isArray(data.features) && data.features.length > 0) {
                data.features.forEach((feature, index) => {
                    let checked = feature.included ? 'checked' : '';
                    container.append(`
                        <div class="input-group mb-2 feature-row">
                            <div class="input-group-text">
                                <input class="form-check-input mt-0" type="checkbox" name="features_included[${index}]" value="on" ${checked}>
                            </div>
                            <input type="text" name="features_text[${index}]" class="form-control" value="${feature.text}" required>
