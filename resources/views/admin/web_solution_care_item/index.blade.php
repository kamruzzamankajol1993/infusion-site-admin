...
@section('title') Website Care Items | {{ $ins_name }} @endsection
@section('css')
<style>
    ...
    .sortable-item-content img { width: 100px; height: 60px; margin-right: 15px; object-fit: cover; border-radius: 4px; }
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Website Care Items</li>
            </ol>
        </nav>
        <div>
            @if (Auth::user()->can('webSolutionCareItemAdd'))
            <button type="button" data-bs-toggle="modal" data-bs-target="#addModal" class="btn text-white" style="background-color: var(--primary-color); white-space: nowrap;">
                <i data-feather="plus" class="me-1" style="width:18px; height:18px;"></i> Add New Care Item
            </button>
            @endif
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
                <li class="nav-item" role="presentation"><a class="nav-link {{ $activeTab === 'table' ? 'active' : '' }}" href="{{ route('webSolution.care-item.index', ['tab' => 'table']) }}" id="table-tab"><i data-feather="list" class="me-1" style="width:16px;"></i> Data Table</a></li>
                @if (Auth::user()->can('webSolutionCareItemUpdate'))
                <li class="nav-item" role="presentation"><a class="nav-link {{ $activeTab === 'reorder' ? 'active' : '' }}" href="{{ route('webSolution.care-item.index', ['tab' => 'reorder']) }}" id="reorder-tab"><i data-feather="move" class="me-1" style="width:16px;"></i> Drag & Drop Reorder</a></li>
                @endif
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade {{ $activeTab === 'reorder' ? 'show active' : '' }}" id="reorder-tab-pane" role="tabpanel" aria-labelledby="reorder-tab">
                    ...
                    @else
                        <div class="alert alert-info text-center">No items found to reorder.</div>
                    @endif
                </div>
            </div>
        </div>
...
@include('admin.web_solution_care_item._partial.addModal')
@include('admin.web_solution_care_item._partial.editModal')
@endsection
@section('script')
<script src="https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js"></script>
@include('admin.web_solution_care_item._partial.script')
@endsection