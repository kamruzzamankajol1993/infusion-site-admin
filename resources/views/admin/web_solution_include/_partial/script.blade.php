<script>
    ...
    var canUpdate = @json(Auth::user()->can('webSolutionIncludeUpdate'));
    var canDelete = @json(Auth::user()->can('webSolutionIncludeDelete'));
    var routes = {
        fetch: "{{ route('webSolution.include.data') }}",
        show: id => `{{ route('webSolution.include.show', ':id') }}`.replace(':id', id),
