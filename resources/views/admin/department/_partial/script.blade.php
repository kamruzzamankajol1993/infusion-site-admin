<script>
    ...
    var canUpdateDepartment = @json(Auth::user()->can('departmentUpdate'));
    var canDeleteDepartment = @json(Auth::user()->can('departmentDelete'));

    var routes = {
        fetch: "{{ route('ajax.departmenttable.data') }}",
        show: id => `{{ route('department.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('department.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('department.destroy', ':id') }}`.replace(':id', id),
        csrf: "{{ csrf_token() }}"
    };
    ...
    $(document).on('click', '.btn-delete', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                const deleteForm = $('#delete-department-form');
    ...
    $('#editUserModal').on('hidden.bs.modal', function () { // Changed to 'hidden.bs.modal'
...
<script>
    var exportInvoicesUrl = "{{ route('downloadDepartmentExcel') }}";
    var exportInvoicesUrlPdf = "{{ route('downloadDepartmentPdf') }}";
