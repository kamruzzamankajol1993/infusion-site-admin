...
@section('title') UK Testimonials | {{ $ins_name }} @endsection
@section('css')
<style>
    ...
    .sortable-item-content img { width: 50px; height: 50px; border-radius: 50%; margin-right: 15px; object-fit: cover; }
...
<div class="container-fluid px-4 py-4">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        ...
        <div>
            @if (Auth::user()->can('ukTestimonialAdd'))
            <button type="button" data-bs-toggle="modal" data-bs-target="#addModal" class="btn text-white" style="background-color: var(--primary-color); white-space: nowrap;">
                <i data-feather="plus" class="me-1" style="width:18px; height:18px;"></i> Add New Testimonial
            </button>
            @endif
        </div>
    </div>
    ...
    <div class="card shadow-sm">
        <div class="card-header bg-white">
            <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
                <li class="nav-item" role="presentation"><a class="nav-link {{ $activeTab === 'table' ? 'active' : '' }}" href="{{ route('ukCompany.testimonial.index', ['tab' => 'table']) }}" id="table-tab"><i data-feather="list" class="me-1" style="width:16px;"></i> Data Table</a></li>
                @if (Auth::user()->can('ukTestimonialUpdate'))
                <li class="nav-item" role="presentation"><a class="nav-link {{ $activeTab === 'reorder' ? 'active' : '' }}" href="{{ route('ukCompany.testimonial.index', ['tab' => 'reorder']) }}" id="reorder-tab"><i data-feather="move" class="me-1" style="width:16px;"></i> Drag & Drop Reorder</a></li>
                @endif
            </ul>
        </div>
        <div class="card-body">
            <div class="tab-content" id="myTabContent">
                ...
                <div class="tab-pane fade {{ $activeTab === 'reorder' ? 'show active' : '' }}" id="reorder-tab-pane" role="tabpanel" aria-labelledby="reorder-tab">
                        ...
                        <ul id="sortableList">
                            ...
                            <li data-id="{{ $item->id }}">
                                <div class="sortable-item-content">
                                    ...
                                    <div><strong>{{ $item->name }}</strong> <small class="text-muted">({{ $item->designation }})</small></div>
                                </div>
                    ...
                    @else
                        <div class="alert alert-info text-center">No testimonials found to reorder.</div>
                    @endif
                </div>
            </div>
        </div>
...
@include('admin.uk_testimonial._partial.addModal')
@include('admin.uk_testimonial._partial.editModal')
@endsection
@section('script')
<script src="https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js"></script>
@include('admin.uk_testimonial._partial.script')
@endsection