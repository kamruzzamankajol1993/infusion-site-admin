<script>
    ...
    var canUpdate = @json(Auth::user()->can('webSolutionWorkItemUpdate'));
    var canDelete = @json(Auth::user()->can('webSolutionWorkItemDelete'));
    var routes = {
        fetch: "{{ route('webSolution.workItem.data') }}",
        show: id => `{{ route('webSolution.work-item.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('webSolution.work-item.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('webSolution.work-item.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('webSolution.workItem.updateOrder') }}",
        token: "{{ csrf_token() }}",
        assetBase: "{{ asset('') }}"
    };
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            res.data.forEach((item, i) => {
                let img = `<img src="${routes.assetBase}${item.image}" alt="Item" style="width: 120px; height: 90px; object-fit: cover;">`;
                let cat = item.category ? item.category.name : '<span class="text-danger">Uncategorized</span>';
                ...
                rows += `<tr>
                    ...
                    <td>${img}</td> <td>${cat}</td> <td>${item.order}</td> <td>${editBtn}${delBtn}</td>
                </tr>`;
            });
            $('#tableBody').html(rows);
    ...
    $(document).on('click', '.sortable', e => { /* ... same as previous script ... */ });
    ...
    $(document.body).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), data => {
            ...
            $('#editVisitLink').val(data.visit_link);
            if (data.image_url) {
