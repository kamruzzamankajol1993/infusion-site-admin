<script>
    ...
    var canUpdate = @json(Auth::user()->can('facebookAdsFaqUpdate'));
    var canDelete = @json(Auth::user()->can('facebookAdsFaqDelete'));
    var routes = {
        fetch: "{{ route('facebookAds.faq.data') }}",
        show: id => `{{ route('facebookAds.faq.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('facebookAds.faq.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('facebookAds.faq.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('facebookAds.faq.updateOrder') }}",
        token: "{{ csrf_token() }}"
    };
    ...
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            if (!items || items.length === 0) {
                 rows = `<tr><td colspan="5" class="text-center text-muted">No FAQs found.</td></tr>`;
            ...
            items.forEach((item, i) => {
                ...
                let answer = $(item.answer).text(); // Strip HTML for table
                answer = answer.length > 80 ? answer.substring(0, 77) + '...' : answer;
                ...
                rows += `<tr>
                    <td>${sl}</td> <td>${item.question || 'N/A'}</td> <td>${answer}</td>
    ...
    $(document.body).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), data => {
            ...
            $('#editAnswer').summernote('code', data.answer);
            editModal.show();
    ...
    $(document).on('click', '.btn-delete', function () {
        const id = $(this).data('id');
        Swal.fire({ title: 'Delete this FAQ?', text: "You won't be able to revert this!", icon: 'warning',
    ...
    $(document).ready(() => {
        ...
        @if (session('error_modal_id') && $errors->update->any())
            ...
            $('#editId').val(failedId);
            $('#editAnswer').summernote('code', `{!! old('answer') !!}`);
            editModal.show();
        @endif
    });
</script>