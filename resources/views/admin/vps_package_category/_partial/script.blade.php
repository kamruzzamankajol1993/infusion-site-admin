<script>
    ...
    var canUpdate = @json(Auth::user()->can('vpsCategoryUpdate'));
    var canDelete = @json(Auth::user()->can('vpsCategoryDelete'));
    var routes = {
        fetch: "{{ route('vpsPage.category.data') }}",
        show: id => `{{ route('vpsPage.category.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('vpsPage.category.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('vpsPage.category.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('vpsPage.category.updateOrder') }}",
        token: "{{ csrf_token() }}"
    };
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            items.forEach((item, i) => {
                ...
                rows += `<tr>
                    <td>${sl}</td> <td>${item.name || 'N/A'}</td> <td>${item.order}</td> <td>${editBtn}${delBtn}</td>
                </tr>`;
            });
            $('#tableBody').html(rows);
    ...
    $(document.body).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), data => {
            $('#editId').val(data.id); $('#editName').val(data.name); editModal.show();
        }).fail(() => Swal.fire('Error!', 'Could not fetch category data.', 'error'));
    });
    $(document).on('click', '.btn-delete', function () {
        const id = $(this).data('id');
        Swal.fire({ title: 'Delete this category?', text: "All packages in this category will also be deleted! This cannot be undone.", icon: 'warning',
