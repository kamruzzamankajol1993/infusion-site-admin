<script>
    ...
    var canUpdate = @json(Auth::user()->can('storeMainBannerUpdate'));
    var canDelete = @json(Auth::user()->can('storeMainBannerDelete'));
    var routes = {
        fetch: "{{ route('ajax.storeMainBanner.data') }}",
        show: id => `{{ route('storeMainBanner.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('storeMainBanner.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('storeMainBanner.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('storeMainBanner.updateOrder') }}",
        token: "{{ csrf_token() }}",
        assetBase: "{{ asset('') }}"
    };
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            if (!items || items.length === 0) {
                 rows = `<tr><td colspan="6" class="text-center text-muted">No banners found.</td></tr>`;
            ...
            items.forEach((item, i) => {
                ...
                let img = `<img src="${routes.assetBase}${item.image}" alt="Banner" style="width: 200px; height: 106px; object-fit: cover;">`;
                ...
                rows += `<tr>
                    <td>${sl}</td> <td>${img}</td> <td><a href="${item.link}" target="_blank">${item.link}</a></td>
                    <td>${status}</td> <td>${item.order}</td> <td>${editBtn}${delBtn}</td>
                </tr>`;
            });
            $('#tableBody').html(rows);
    ...
    $(document.body).on('click', '.btn-edit', function () {
        ...
        $.get(routes.show(id), data => {
            ...
            editModal.show();
        }).fail(() => Swal.fire('Error!', 'Could not fetch banner data.', 'error'));
    });
    
    $(document).on('click', '.btn-delete', function () {
        const id = $(this).data('id');
        Swal.fire({ title: 'Delete this banner?', text: "You won't be able to revert this!", icon: 'warning',
