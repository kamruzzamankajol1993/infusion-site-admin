<script>
    ...
    var canUpdate = @json(Auth::user()->can('graphicDesignChecklistUpdate'));
    var canDelete = @json(Auth::user()->can('graphicDesignChecklistDelete'));
    var routes = {
        fetch: "{{ route('graphicDesign.checklist.data') }}",
        show: id => `{{ route('graphicDesign.checklist.show', ':id') }}`.replace(':id', id),
        update: id => `{{ route('graphicDesign.checklist.update', ':id') }}`.replace(':id', id),
        delete: id => `{{ route('graphicDesign.checklist.destroy', ':id') }}`.replace(':id', id),
        updateOrder: "{{ route('graphicDesign.checklist.updateOrder') }}",
        token: "{{ csrf_token() }}"
    };
    ...
    function fetchData() {
        $.get(routes.fetch, { page: currentPage, search: searchTerm, sort: sortColumn, direction: sortDirection }, res => {
            ...
            // Note: 'res.data.data' is correct if using default pagination JSON
            let items = res.data || []; 
