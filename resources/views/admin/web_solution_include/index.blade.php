...
@section('title') Service Includes | {{ $ins_name }} @endsection
@section('css')
<style>
    ...
    /* Updated image style for reorder list */
    ...
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                ...
                <li class="breadcrumb-item active" aria-current="page">Service Includes</li>
            </ol>
        </nav>
        <div>
            @if (Auth::user()->can('webSolutionIncludeAdd'))
            <button type="button" data-bs-toggle="modal" data-bs-target="#addModal" class="btn text-white" style="background-color: var(--primary-color); white-space: nowrap;">
                <i data-feather="plus" class="me-1" style="width:18px; height:18px;"></i> Add New Item
            </button>
            @endif
        </div>
    </div>
        ...
        <div class="card-header bg-white">
            <ul class="nav nav-tabs card-header-tabs" id="myTab" role="tablist">
                <li class="nav-item" role="presentation">
                    <a class="nav-link {{ $activeTab === 'table' ? 'active' : '' }}" href="{{ route('webSolution.include.index', ['tab' => 'table']) }}" id="table-tab">
                        <i data-feather="list" class="me-1" style="width:16px;"></i> Data Table
                    </a>
                </li>
                @if (Auth::user()->can('webSolutionIncludeUpdate'))
                <li class="nav-item" role="presentation">
                     <a class="nav-link {{ $activeTab === 'reorder' ? 'active' : '' }}" href="{{ route('webSolution.include.index', ['tab' => 'reorder']) }}" id="reorder-tab">
...
@include('admin.web_solution_include._partial.addModal')
@include('admin.web_solution_include._partial.editModal')
@endsection
@section('script')
<script src="https://cdn.jsdelivr.net/npm/sortablejs@latest/Sortable.min.js"></script>
@include('admin.web_solution_include._partial.script')
@endsection